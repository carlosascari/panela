...
const etcHosts = onepath.require('~/hosts');
const NginxConfig = onepath.require('~/nginxc');
const { isObject, isString } = onepath.require('~/util');
const {
...
class Panela {
  ...
  */
  constructor() {
    Object.defineProperty(this, '__PANELA__', {
      value: {
        builders: [],
        databases: [],
        engines: [],
  ...
  */
  listen(options={}) {
    ...
    this.hosts().forEach(host => {
      ...
      etcHosts.set('127.0.0.1', host.name);
      ...
      httpClause.clause('server', cl => {
        ...
        .directive('root', host.root)
      })

      console.log(bold.green(host.name));
